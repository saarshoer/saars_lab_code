...
read_len = {'NebNext': 100, 'NebNext_new': 100,  # Gut 10K
            'Hanson': 75, 'NovaSeq': 100,  # Gut
            'Nextseq': 75, 'Novaseq': 100}  # Oral
read_len = read_len[run_name]

urb_num_mapped_to_subsample = {'NebNext': 5000000, 'NebNext_new': 5000000,  # Gut 10K
                               'Hanson': 8000000, 'NovaSeq': 5000000,  # Gut
                               'Nextseq': 4000000, 'Novaseq': 2500000}  # Oral
urb_num_mapped_to_subsample = urb_num_mapped_to_subsample[run_name]

urb_min_mapped_to_retain = {'NebNext': 1000000, 'NebNext_new': 1000000,  # Gut
                            'Hanson': 1000000, 'NovaSeq': 1000000,  # Gut
...
if study_ids == ['10K']:
    samples = loader().get_data('segata_species', study_ids=study_ids).df_metadata.groupby('RegistrationCode').filter(
        lambda p: (len(p) == 2) & (p['research_stage'].unique().tolist() == ['baseline', '02_00_visit'])).index
else:
    samples = loader().get_data('segata_species', study_ids=study_ids).df.index

if body_site == 'Gut' and study_ids == ['PNP3']:
